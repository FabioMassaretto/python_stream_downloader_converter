...
from modules.pytube3 import __version__, CaptionQuery, Stream, Playlist
from modules.pytube3 import YouTube
from modules.pytube3.exceptions import PytubeError
from modules.pytube3.helpers import safe_filename, setup_logger
...
def _print_available_captions(captions: CaptionQuery) -> None:
    print(f"Available caption codes are: {
          ', '.join(c.code for c in captions)}")
...
def download_audio(
    ...
    audio = (
        youtube.streams.filter(
            only_audio=True, subtype=filetype).order_by("abr").last()
    )
